                ...
                <div>
                  ...
                  <div className="border hairline divide-y">
                    {stats.top_visited.length === 0 ? (
                      ...
                      <button key={p.id} onClick={() => { rememberModalReturn(); setModalNavOrder(stats.top_visited.map((x) => x.id)); navigate(refPath(p.id, p.title)); }} className="w-full flex items-center gap-3 p-3 text-left hover:bg-secondary/50 transition-colors">
                ...
                <div>
                  ...
                  <div className="border hairline divide-y">
                    {stats.top_bookmarked.length === 0 ? (
                      ...
                      <button key={p.id} onClick={() => { rememberModalReturn(); setModalNavOrder(stats.top_bookmarked.map((x) => x.id)); navigate(refPath(p.id, p.title)); }} className="w-full flex items-center gap-3 p-3 text-left hover:bg-secondary/50 transition-colors">
                        <span className="font-mono text-[10px] text-muted-foreground w-5">{(i + 1).toString().padStart(2, "0")}</span>
                        <div className="w-12 h-8 bg-secondary shrink-0 overflow-hidden">
                          {p.thumbnail_url && <img src={p.thumbnail_url} alt={p.title} className="w-full h-full object-cover" />}
                        </div>
