...
function CannesLionsScraper() {
  ...
  async function run() {
    ...
    try {
      ...
      if (cFilm) categories.push("film", "craft");
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (ev.type === "progress") {
              ...
              const s = ev.summary;
              setLog((l) => [...l, { kind: "done", text: `✓ Complete — ${s.total_fetched} fetched · ${s.auto_published} auto-published · ${s.sent_to_review} in review queue · ${s.skipped_duplicates} duplicates skipped` }]);
  ...
  return (
    <section>
      <p className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground mb-2">⏵ Scrapers</p>
      <h2 className="text-3xl font-black tracking-tighter mb-1 font-serif">✦ Cannes Lions — Grand Prix &amp; Gold Lions</h2>
      <p className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground mb-6">
        Scrapes lovethework.com for award winners by year and category
      </p>
