...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      let itemsByFolder: Record<string, string[]> = {};
      let refsById: Record<string, Reference> = {};
      if (folderIds.length) {
  ...
  useEffect(() => {
    if (profile) {
      document.title = `@${profile.username} — The Creatives Room`;
    }
  }, [profile]);
    ...
    <div className="min-h-screen grain">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-5 flex flex-wrap items-center gap-4">
                <span className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
                  {profile.public_folders_count ?? 0}{" "}
                  {profile.public_folders_count === 1 ? "public collection" : "public collections"}
                </span>
                <span className="font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
                  · {profile.submitted_count ?? 0}{" "}
                  {profile.submitted_count === 1 ? "submission" : "submissions"}
                </span>
                <button
                  onClick={handleShare}
                  className="ml-auto md:ml-0 inline-flex items-center gap-1.5 px-3 py-1.5 border hairline font-mono text-[10px] uppercase tracking-widest hover:bg-secondary"
                >
      ...
      <main className="container py-12 space-y-16">
        <section>
          ...
          {folders.length === 0 ? (
            <p className="font-display text-2xl italic text-muted-foreground">No public collections yet.</p>
          ) : (
