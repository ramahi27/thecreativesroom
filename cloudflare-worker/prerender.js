// OG prerender worker for /ref/* on thecreativesroom.com
// Deploy as a Cloudflare Worker with a route: thecreativesroom.com/ref/*
// (and www.thecreativesroom.com/ref/* if you use www).
//
// What it does: when a social/search bot requests /ref/<uuid>-<slug>,
// it fetches the reference from Supabase and returns an HTML page with
// proper OG/Twitter meta tags so WhatsApp/Telegram/etc. show the
// reference's actual thumbnail. Real users are passed through to the
// SPA via `fetch(request)` (origin).
...
const BOT_RE =
  /facebookexternalhit|Facebot|Twitterbot|LinkedInBot|WhatsApp|TelegramBot|Slackbot|Discordbot|Googlebot|bingbot|Applebot|DuckDuckBot|ia_archiver|Pinterest|redditbot|python-requests|curl\/|wget\//i;
...
async function deriveThumb(sourceUrl) {
...
export default {
  async fetch(request) {
    ...
    // Only handle /ref/* — anything else, pass to origin.
    if (!url.pathname.startsWith("/ref/")) return fetch(request);

    // Real users → origin (the SPA handles the route).
    if (!BOT_RE.test(ua)) return fetch(request);

    const segment = url.pathname.replace(/^\/ref\//, "");
    ...
    const thumb = await deriveThumb(ref.source_url);
    const ogImage = ref.thumbnail_url || thumb || DEFAULT_OG_IMAGE;
    ...
    const canon = escape(canonicalUrl);

    const schemaType =
      ref.type === "video" ? "VideoObject" :
      ref.type === "image" ? "ImageObject" : "CreativeWork";
...
<body>
  <p>Redirecting to <a href="${canon}">${title}</a>…</p>
</body>
</html>`;
